...
@description('Optional. If the array is null then value would be set to 0 - no availability zones to be used. Must pass the zone leverage either pickZones function or pass down the array with zone number')
...
module vm_desiredStateConfigurationExtension 'extensions/deploy.bicep' = if (extensionDSCConfig.enabled) {
  ...
  params: {
    ...
    type: 'DSC'
    typeHandlerVersion: contains(extensionDSCConfig, 'typeHandlerVersion') ? extensionDSCConfig.typeHandlerVersion : '2.73'
